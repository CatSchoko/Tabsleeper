...
Open chrome://extensions (also works in Edge, Brave, and other Chromium browsers)
...
Reads the tab's URL, title, favicon, and (on YouTube) current video timestamp
Encodes all of that into the query string of a tiny internal sleep.html page
Navigates the tab there — the original page is completely torn down, no residual state
On Wake, rebuilds the original URL (with the YouTube timestamp appended, if applicable) and navigates back
